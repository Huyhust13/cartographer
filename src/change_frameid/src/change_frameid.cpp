...
Scan2::Scan2()
{
	...
	scan_sub = n.subscribe<sensor_msgs::LaserScan>("/slamware_ros_sdk_server_node/scan",1, &Scan2::scanCallBack, this); 
}
...
void Scan2::scanCallBack(const sensor_msgs::LaserScan::ConstPtr& scan2)
{
  ...
  scan.header.seq = scan2->header.seq;
	scan.header.stamp = scan2->header.stamp;

	if(scan2->header.frame_id == "/laser"){
    ...
    scan.scan_time = scan2->scan_time;
	scan.range_min = scan2->range_min;
	scan.range_max = scan2->range_max;
...
int main(int argc, char** argv){
	ros::init(argc, argv, "change_frameid");
	Scan2 scan2;
	ros::spin();
}